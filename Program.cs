    ...
    class Program
    {
        ...
        private static byte[] FindSaltFile(byte[] clientHvSalt, int keySelect)
        {
            try
            {
                ...
                //saltDump = File.ReadAllBytes($"Seeds/" + keySelect + "/Salts/" + Utils.BytesToHexString(clientHvSalt) + ".bin");

                 saltDump = File.ReadAllBytes("Seeds//" + keySelect + "//" + "0x" + Utils.BytesToHexString(clientHvSalt) + ".bin");
        ...
        private static bool HandleXeKeysExecuteResponse(TcpClient client, byte[] data, NetworkStream Stream, byte[] cpuKey, List<Log.PrintQueue> logId)
        {
            try
            {
                ...
                Log.Add(logId, ConsoleColor.Cyan, "Info", string.Format("KVCpu Key: {0}", Utils.BytesToHexString(cpukeyFromKV)), ipAddress);
                ...
                Buffer.BlockCopy((typeOneKv ? BitConverter.GetBytes((ushort)0xD81E).Reverse().ToArray() : BitConverter.GetBytes((ushort)0xD83E).Reverse().ToArray()), 0, responseBuff, 0x2E, 0x2);
                ...
                //Log.Add(logId, ConsoleColor.Magenta, "Info", string.Format("Buffer: {0}", Utils.BytesToHexString(responseBuff)), ipAddress);
        ...
        private static void ClientConnectedThread(object arg)
        {
            ...
            if (Utils.AuthorizedIP(client) == false)
            {
                ...
                {
                    LastBannedIP = ipAddress;
                    Utils.Shell(string.Format("netsh advfirewall firewall add rule name=" + "BLOCK IP ADDRESS - BAD_IP_BAN@{0}" + "dir=in action=block remoteip={0}", ipAddress));
                    ...
                    //netsh advfirewall firewall add rule name="BLOCK IP ADDRESS -  184.105.139.67" dir=in action=block remoteip=184.105.139.67
        ...
        static void Main(string[] args)
        {
            ...
            // API IP Adress
            ServerIP = File.ReadAllText("settings.ini");
